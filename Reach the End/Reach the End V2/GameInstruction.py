...
def create_game(configurations, map_id = [], view_distance = ()):
    ...
    for layer in world_data :
        ...
        for cell in layer :
            ...
            if "turret" in cell :
                ...
                GameWorld.place("condition", x, y, white_space_remove(properties["sprite"]), name_of = cell, has_condition = properties["has_condition"], view_distance_n = properties["view_distance_n"], condition_blocks = properties["condition_blocks"])
            x += 1
        y += 1
    #firing groups
    ...
    for key, value in gamedata.items() :
        if "condition" in key :
            for layer in map_id :
                if key in layer :
                    GameWorld.conditions_in_map.append(value["has_condition"])
    for key, value in gamedata.items() :
        if "condition" in key :
            if value["has_condition"] in GameWorld.conditions_in_map :
                for block in value["condition_blocks"] :
                        GameWorld.world[block[1]][block[0]].has_condition = True
    return GameWorld
...
def shoot_procedure (World, tick) :
    ...
    for cordinate in turrets :
        cell = World.world[cordinate[1]][cordinate[0]]
        if cell.name in World.firing_groups[0] :
...
class World :
    ...
    def display(self):
        ...
        sys.stdout.flush()
    def place (self, name, x, y, sprite, health = 0, name_of = "", health_sprite = "", consumed_health_consumed = "", has_condition = None, view_distance_n = None, condition_blocks = []):
        if name == "player" :
            ...
            self.world[y][x] = ConditionCell(x, y, name_of, sprite, has_condition, view_distance_n, condition_blocks)
...
class ConditionCell :
    def __init__(self, x, y, name, sprite, has_condition, view_distance_n, condition_blocks):
        self.type = "condition"
        self.sprite = sprite
        self.is_over_writeable = False
        self.is_walkable = True
    ...
    def move (self, World, direction, stage_start_time) :
        ...
        try :
            ...
            if cell_to_move.type == "empty" :
                ...
                for block in cell_to_move.condition_blocks :
                    x, y = block
                    World.world[y][x].has_condition = False
                    if world[y][x].type == "wall" :
                        World.world[y][x] = EmptyCell(x, y, "  ")
                world[cell_to_move.y][cell_to_move.x] = Player(cell_to_move.x, cell_to_move.y, self.sprite, self.health, self.health_sprite, self.consumed_health_consumed)
                world[self.y][self.x] = EmptyCell(self.x, self.y, sprite = "  ")
                self.x , self.y = cordinates_to_move
...
def start_game (configurations, map_id = "", view_distance = ()) :
    ...
    for condition in GameWorld.conditions_in_map :
